...
// Check if API informations are filled
function check_wc_api_filled(){
	ds_call("CheckIfWCApiFilled", function(output){
		if(!output){
			ds_call("GenWcLink", function(link){
				Swal.fire({
				  type: 'error',
				  title: 'Oups...',
...
ds_call("CheckIfConnected", function(status){
	if(status){
		...
		if(scriptParams.ds_username){
			...
			form.append("username", scriptParams.ds_username);
			form.append("password", scriptParams.ds_password);
			...
			xhr.addEventListener("readystatechange", function () {
			  if (this.readyState === this.DONE) {
			    ...
			    if(msg.response === "ok"){
					...
					document.getElementById("ds_successbox").setAttribute("style", "display: none");
					document.getElementById("ds_errorbox").removeAttribute("style");
					loader("hide");
					// Check if on config page and if not redirect
				}
			  }
			});
...
function loader(arg){
	if(arg == "show"){
		document.getElementById("ds_loader").removeAttribute("style");
	}
	else{
		document.getElementById("ds_loader").setAttribute("style", "display: none");
	}
}

function show_login_form(){
	document.getElementById("ds_custom_id").setAttribute("style", "display: none");
	document.getElementById("ds_config_form").removeAttribute("style");
}
...
function ds_config_submit(event) {

	loader("show");

	event.preventDefault();
	var username = document.getElementById("dropshifty_username").value;
	var password = document.getElementById("dropshifty_password").value;
	...
	xhr.addEventListener("readystatechange", function () {
	  if (this.readyState === this.DONE) {
	  	var msg = JSON.parse(this.responseText);
	    if(msg.response === "ok"){
			document.getElementById("ds_config_form").submit();
			loader("hide");
		}	
		else{
			document.getElementById("ds_successbox").setAttribute("style", "display: none");
...
function ds_product_submit(event){
...
function fetch_product(supplier, sku, handledata){
	...
	loader("show");
	var url;
	ds_call("GetCbToken", function(output){
